...
void main(void) {
    ...
    signed char dd = 26;
    char dm = 3;
    unsigned int dy = 2019;
    char dBST = 0;
    unsigned int min_length = 1;
    unsigned int hour_length = 1;
    unsigned int hours_in_day = 24;
    ...
    while (1){
        ...
        if (ds>=min_length){
            ds = 0;
           if (dmin<(hour_length-1)) {
                dmin ++;
           }
           else if (dmin>=(hour_length-1)){
               dmin = 0;
                if (dh<(hours_in_day-1)) {
                    dh++;
                }
                else if (dh>=(hours_in_day-1)){
                    dh = 0;
                   if ((dname<6)&& (dd<dim[dm-1])){
                        dname ++;
                        dd ++;
                   }
        ...
        LCD_setlines(dname,dh,dmin,ds,dd,dm,dy,dBST);
        LEDarray_disp_bin(dh);
        LDR_val();
        LDR_thres(LDR_val(),dh);
    }
}
